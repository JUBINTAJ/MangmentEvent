...
import {Bell,ChevronLeft,ChevronLeftCircle,LayoutGrid,Menu,X,} from "lucide-react";
    ...
    <div className="min-h-screen bg-gradient-to-br from-[#010002] via-[#280F3C] to-[#60218B] text-white relative">
      <header className="flex justify-between items-center p-4  ">
        <div className="flex items-center">
          <button
            onClick={toggleMenu}
            className="lg:hidden p-2 text-pink-300 hover:text-pink-100"
          >
            <Menu size={24} />
          </button>
      ...
      <div className="flex flex-col lg:flex-row h-[calc(100vh-73px)]">
        <aside
          className={`fixed inset-y-0 left-0  lg:relative lg:w-[220px] border-r border-pink-500/20 transition-transform duration-300 z-50 ${
            menuOpen
              ? "translate-x-0 w-1/2 max-w-[300px]"
        ...
        <main className="flex-1 p-4 overflow-auto ml-5 border-1 border-[#D960B6] rounded-xl scrollb  ">
          <Eventright />
        </main>
      </div>
    </div>
